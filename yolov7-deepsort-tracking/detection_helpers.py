        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                if plot_bb:  # Add bbox to image   # save_img
                    ...
                    plot_vert_dot_line(left_bound, im0)
                    plot_vert_dot_line(right_bound, im0)
                    if cls == 11:
                        x, y = ((bbox[2]+bbox[0])/2),((bbox[3]+bbox[1])/2)
                        cv2.circle(im0, (int(x),int(y)), radius=5, color=[255,150,0], thickness=-1)
                        if (x < left_bound):
                            cv2.putText(im0, 'LEFT', (50,50), cv2.FONT_HERSHEY_COMPLEX, 1.5, [0,0,0], 1, cv2.LINE_AA)
                            print("STOP SIGN IS LEFT OF MIDDLE")
                        elif (x > right_bound):
                            cv2.putText(im0, 'RIGHT', (50,50), cv2.FONT_HERSHEY_COMPLEX, 1.5, [0,0,0], 1, cv2.LINE_AA)
                            print("STOP SIGN IS RIGHT OF MIDDLE")
                        else:
                            cv2.putText(im0, 'MIDDLE', (50,50), cv2.FONT_HERSHEY_COMPLEX, 1.5, [0,0,0], 1, cv2.LINE_AA)
