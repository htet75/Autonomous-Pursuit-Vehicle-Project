...
class YOLOv7_DeepSORT:
    ...
    def track_video(self,video:str, output:str, skip_frames:int=0, show_live:bool=False, count_objects:bool=False, verbose:int = 0):
        ...
        if output: # get video ready to save locally if flag is set
            width = 600  # by default VideoCapture returns float instead of int
            height = 400
            fps = 24
        ...
        while True: # while video is running
            return_value, depth_frame, frame = vid.get_frame()
            if not return_value:
            ...
            if count_objects:
                cv2.putText(frame, "Objects being tracked: {}".format(count), (400, 550), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.5, (0, 0, 0), 2)
