...
def rc_control(bbox, depth_frame, width):
    """
    """
    ...
    print("steering: {} || x_value : {} || distance: {:.2f}".format(steering, int(x), float(distance)))
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None, category = None):
    """
...
class YoLov7TRT(object):
    ...
    def execute(self, image):
        ...
        keypoints = self.get_keypoints(image, self.human_pose, self.topology, counts, objects, peaks)
        # print(keypoints)
...
class inferThread(threading.Thread):
    ...
    def run(self):
        ...
        out = None
        width = 640  # by default VideoCapture returns float instead of int
        height = 480
        ...
        while(True):
            ...
            ret, depth_frame, frame = self.cap.get_frame()
            # img = cv2.resize(frame[:,240:1680], (640, 480))
            img = frame
            result, use_time = self.yolov7_wrapper.infer(img, depth_frame)
            frames += 1
            

            print(frames/(time.time()-start_time))
            cv2.putText(result, 'FPS: {:.2f}'.format(frames/(time.time()-start_time)), (width-int(width/7),20), cv2.FONT_HERSHEY_COMPLEX, 0.5, [0,0,0], 1, cv2.LINE_AA)
